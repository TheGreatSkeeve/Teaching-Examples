...
# So, with all this in mind, let's write a short program that uses everything from the above.
# Let's say this one is for entering employee data, and then displaying it.


# Initialize an empty list to hold the employee data
employeeData = []

'''
Basic function to minimal simplify asking the user for data

Instead of doing something like 
    firstname = input("What is the user's first name")
We can use this function to do:
    firstname = getData("User First Name")
'''
def getData(value):
    data = input("Please enter the value for: " + value)
    return data


'''
Function to make a username from first name last name, FirstInitialLastName format

First it makes all the letters lowercase for both names
Then it takes the first letter of the first name using [0] and adds the last name onto that

That matches our format, so it returns that value
'''
def makeUsername(first,last):
    first_lower = first.lower()
    last_lower = last.lower()
    name = first_lower[0]+last_lower
    return name


'''
This is a function that will do all the work to add a new employee.

It will call on getData to pull the information, and then 
    makeUsername to create a username from the provided first name 
    and last name.  

Then it returns a list of the employee's info
'''
def newEmployee():
    firstName = getData("Employee First Name")
    lastName = getData("Employee Last Name")
    phoneNumber = getData("Employee Cell Phone")
    username = makeUsername(firstName,lastName)
    return [firstName,lastName,phoneNumber,username]

# This will be a data validator of sorts.
# We'll check the length and type of the phone number to make sure it's a valid phone number
#
def checkData(employee):
    if len(phoneNumber) ==